...
if [ "$STATIC_COUNT" = "0" ]; then
  MSG="NO SITE TO SYNC"
  echo $MSG
  $SCRIPT_PATH/bf-tome-status-indicator-update.sh "$TR_START_TIME" "$MSG"
  exit 1;
fi;
...
if [ "$TOME_PUSH_NEW_CONTENT" == "1" ]; then
  ...
  $SCRIPT_PATH/bf-tome-status-indicator-update.sh "$TR_START_TIME" "Static Site Generation Completed Successfully, but sync to S3 failed or was not attempted"
fi

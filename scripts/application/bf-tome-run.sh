...
echo "Running static site build..." | tee -a $TOMELOG
set -o pipefail  # Need to capture tome-static failure on next line.
$SCRIPT_PATH/bf-tome-status-indicator-update.sh "$TR_START_TIME" "Static Site Generation Started"
$SCRIPT_PATH/bf-tome-static.sh $URI 2>&1 | tee -a $TOMELOG
TOME_SUCCESS=$?
if [ "$TOME_SUCCESS" == "0" ]; then
  $SCRIPT_PATH/bf-tome-sync.sh $TOMELOGFILE $YMDHMS $FORCE
else
  ...
  $SCRIPT_PATH/bf-tome-status-indicator-update.sh "$GEN_FAIL_TIME" "Static Site Generation Failed"
#   exit 1
#   fi
# else
